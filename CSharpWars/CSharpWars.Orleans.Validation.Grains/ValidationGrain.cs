...
public class ValidationGrain : Grain, IValidationGrain
{
    ...
    public ValidationGrain(
        ...
        ILogger<ValidationGrain> logger)
    {
    ...
    public async Task<ValidatedScriptDto> Validate(ScriptToValidateDto scriptToValidate)
    {
        ...
        {
            _logger.LogError(ex, "An error occurred during script validation.");

            return new ValidatedScriptDto
            {
                CompilationTimeInMilliseconds = 0,
                RunTimeInMilliseconds = 0,
                ValidationMessages = new List<ValidatedScriptMessageDto>
                {
                    new ValidatedScriptMessageDto
                    {
                        Message = ex.Message
                    }
                }
